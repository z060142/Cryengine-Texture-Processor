...
For this converter path, `.tif` and `.dds` are treated as the same practical CE
texture asset identity. The gate groups outputs by base name and CE suffix, not
by extension, so `wall_diff.tif` and `wall_diff.dds` occupy the same `diff`
output slot. The material resolver uses the same identity rule when probing
processed outputs: `tif` probing also accepts the matching `dds` file, and `dds`
probing also accepts the matching `tif` file.

The UI texture export format is restricted to `tif`, because the current
exporters write TIFF source textures and optional CryEngine DDS files are
generated by the separate DDS post-process. Old or external settings that ask
for `png`/`tga` are normalized away by the model texture resolver and fall back
to `tif` probing, which also accepts the matching `dds` file for the same CE
texture slot.
