...
M_eff = T(C1) × T(C2) × T(C3)          ← 2026-07-27 勘誤：無開頭 m 因子！
  T(Ci) = smoothstep(cut−w, cut+w, value)；w = 過渡半寬，預設 0.05，0 = 硬切

  勘誤說明：初版誤寫 `m × ∏T`，會讓通過 gate 的中間值 metallic（如 0.7）
  仍以 0.7 做半轉換而落死區——正是本案要消滅的情境。m 只應透過 T(C1)
  進入判定：過線 → M_eff≈1 全轉，不過 → 0 不轉。這才是「轉好轉滿」。
```
...
## 4. 閾值定錨：PBR 規範為主（2026-07-27 業主裁決），目測值降為校準參考

1. **M_cut = 0.5，過渡半寬 0.05（PBR 教義）**：Substance《Comprehensive
   PBR Guide》明言 metallic 本質上應為二值（0/1），中間值只該出現在
   材質交界的過渡像素。二值化 gate 不是權衡，**正是 PBR 規範本身**——
   本設計等於把不規範素材強制正規化回 PBR。
2. **S_min = 180 sRGB（PBR 金屬 F0 下限）**：真實金屬 F0——鐵 ≈196、
   金 ≈204、鋁 ≈245；Substance 驗證圖表對「髒/色偏金屬」的容忍下限
   ≈155。預設取 **180**（容忍線與鐵之間）；`155`（寬鬆）與 `220`
   （業主粗實驗嚴格值）記錄為備選。低於此線的「金屬」（鏽、氧化面）
   按 PBR 本就該是介電質——C3 濾鏽與 PBR 完全同向。
3. **G_cut 預設關閉（PBR 不設此限）**：PBR 中粗糙金屬完全合法（拉絲鋼
   roughness 0.5 是教科書案例），gloss 不該參與金屬判定。業主粗實驗的
   0.9 反映的是 CE Illum 特定表現，保留為**選配 CE 經驗開關**
   （`metal_gate_gloss_cut`，預設 0 = off），swatch 目測後若證實 CE
   確實撐不起粗金屬再啟用。

**swatch atlas 仍要做**，但角色從「定閾值」改為「驗證 PBR 預設在 CE 內
的實際表現 + 日後迴歸樣本」：橫軸 metallic 0→1、縱軸 gloss 0→1、
多行 basecolor（白/鐵灰/金/鏽橘），轉換後進引擎目測。
